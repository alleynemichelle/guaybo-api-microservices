...
import { ValidateUndefinedStringsInterceptor } from 'apps/libs/common/api/interceptors/validate-undefined-strings.interceptor';
...
async function bootstrap(): Promise<any> {
    const app = await NestFactory.create(HostsModule);
    ...
    app.useGlobalInterceptors(new ValidateUndefinedStringsInterceptor());
    app.useGlobalPipes(
        new ValidationPipe({
            transform: true,
            whitelist: true,
    ...
    if (process.env.STAGE === 'dev') {
        const swaggerDoc = setSwaggerConfig(app, 'Hosts');
